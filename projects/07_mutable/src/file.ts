...
import type { MutableCapability, Context, FlatRecord, Record } from './types.js';
...
export class FileCapability implements MutableCapability {
  ...
  async createRecord(path: string, content?: Buffer): Promise<void> {
    ...
    // Ensure intermediate contexts
    const parts = path.split('/').filter(Boolean);
    parts.pop(); // remove the key itself
    let current: StoreRoot | ContextNode = root;
    for (const part of parts) {
  ...
  async writeRecord(path: string, content: Buffer): Promise<void> {
    ...
    if (!parent.children[key] || parent.children[key].type !== 'file') {
      throw new Error(`Not a file record: ${path}`);
    }

    parent.children[key] = { type: 'file', content: content.toString('base64') };
    await this.save(root);
  }
